const logger = require('pomelo-logger').getLogger('bearcat-treasures', 'PlayerHandler');
...
class Handler {
     ...
     *
     * @public
     * @param {{name: string, playerId: number}} msg
     * @param {object} session
     * @param {function} next
     */
    enterScene(msg, session, next) {
        const role = this.app.get('dataApi').role.random();
        const player = bearcat.getBean('player', {
            id: msg.playerId,
            name: msg.name,
            kindId: role.id
        });
